...
# Function to extract vectorized train/validation/test datasets 
# from Pandas series and already trained doc2vec model
def extract_doc2vec_vecs(train_series, val_series, test_series, vect):
    
    docs_train = list(train_series)
    docs_val = list(val_series)
    docs_test = list(test_series)
    
    X_train = np.array([vect.infer_vector(docs_train[ind].split()) for ind in range(len(docs_train))])
    X_val = np.array([vect.infer_vector(docs_val[ind].split()) for ind in range(len(docs_val))])
    X_test = np.array([vect.infer_vector(docs_test[ind].split()) for ind in range(len(docs_test))])
